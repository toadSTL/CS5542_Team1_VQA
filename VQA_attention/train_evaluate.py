...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=200,
                       help='Expochs')
    parser.add_argument('--max_steps', type=int, default=11000,
                       help='max steps, set 1 for evaluating the model')
    parser.add_argument('--version', type=int, default=1,
                       help='VQA data version')
    parser.add_argument('--sample_every', type=int, default=540,
                       help='Debug every x iterations')
    parser.add_argument('--evaluate_every', type=int, default=2700,
    ...
    conv_features_train, image_id_list_train = data_loader.load_conv_features('train', args.cnn_model, args.feature_layer)
    image_id_map_train = {image_id_list_train[i] : i for i in range(len(image_id_list_train))}
    ...
    for epoch in range(args.epochs):
        ...
        while (batch_no*args.batch_size) < len(qa_data['training']):
            ...
            if step % args.evaluate_every == 0|step == 1:
                ...
                train_acc = evaluate_model_train(model, qa_data, args, model_options, sess, conv_features_train, image_id_map_train)
                print("Train_ACCURACY>> ", train_acc, step, epoch)
                training_log.append({
                    'step' : step,
                    'epoch' : epoch,
...
def evaluate_model_train(model, qa_data, args, model_options, sess, conv_features, image_id_map):
    # to avoid h5py from slowing down.
    # conv_features = data_loader.load_conv_features('train', args.cnn_model, args.feature_layer,
    #                                                load_image_list=False)
    ...
    while (batch_no * args.batch_size) < len(qa_data['training']):
        ...
        question, answer, image_features, image_ids, ans_freq_batch = get_batch(
            batch_no, args.batch_size, qa_data['training'],
            conv_features, image_id_map, 'trian', model_options
        )
        ...
        acc_train = np.sum(prediction_check, dtype="float32") / len(prediction_check)
        print("Eavluating", batch_no, len(qa_data) / args.batch_size, acc_train)
        batch_no += 1
...
def evaluate_model(model, qa_data, args, model_options, sess, conv_features, image_id_map):
    ...
    while (batch_no*args.batch_size) < len(qa_data['validation']):
        ...
        for bi, pred_ans in enumerate(pred_ans_text):
            ...
            if pred_ans in ans_freq_batch[bi]:# and ans_freq_batch[bi][pred_ans] >= 3:
                prediction_check.append(min(1.0, ans_freq_batch[bi][pred_ans]/3.0))
                # prediction_check.append(1.0)
                # image_array = utils.image_array_from_image_id(image_ids[bi], 'val')
                question_text = utils.question_indices_to_text(question[bi], ques_vocab_rev)
                match = True
                score = min(1.0, ans_freq_batch[bi][pred_ans]/3.0)
                batch_val_data['question'] = question_text
                batch_val_data['actual_answer'] = actual_ans_text[bi]
                batch_val_data['predicted_answer'] = pred_ans_text[bi]
                batch_val_data['image_id'] = image_ids[bi]
                batch_val_data['answer_correct?'] = match
                batch_val_data['score'] = score
                batch_val_data['batch_no'] = batch_no
                batch_val_data['batch_index'] = bi
            else:
                prediction_check.append(0.0)
                # image_array = utils.image_array_from_image_id(image_ids[bi], 'train')
                question_text = utils.question_indices_to_text(question[bi], ques_vocab_rev)
                batch_val_data['question'] = question_text
                batch_val_data['actual_answer'] = actual_ans_text[bi]
                batch_val_data['predicted_answer'] = pred_ans_text[bi]
                ...
                batch_val_data['batch_index'] = bi
            # print pred_ans, ans_freq_batch, prediction_check[-1]
            validation_data.append(batch_val_data)
    ...
    f = open('Data/validation_data.json', 'w', encoding="utf8")
