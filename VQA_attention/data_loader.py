...
def prepare_training_data(version=2, data_dir='Data', options={}):
    if version == 1:
        ...
        t_q_json_file = join(data_dir, 'v2_mscoco_train2014_annotations.json')
        t_a_json_file = join(data_dir, 'v2_mscoco_val2014_annotations.json')

        v_q_json_file = join(data_dir, 'v2_OpenEnded_mscoco_train2014_questions.json')
        v_a_json_file = join(data_dir, 'v2_OpenEnded_mscoco_val2014_questions.json')
        qa_data_file = join(data_dir, 'qa_data_file1.json')
        vocab_file = join(data_dir, 'vocab_file1.json')
    ...
    with open(qa_data_file, 'w', encoding="utf8") as f:
