...
def prepare_training_data(version=2, data_dir='Data', options={}):
    if version == 2:
        t_q_json_file = join(data_dir, 'test2.json')
        t_a_json_file = join(data_dir, 'test1.json')

        v_q_json_file = join(data_dir, 'MultipleChoice_abstract_v002_val2015_questions.json')
        v_a_json_file = join(data_dir, 'abstract_v002_val2015_annotations.json')
        qa_data_file = join(data_dir, 'qa_data_file2_test.json')
        vocab_file = join(data_dir, 'vocab_file2_test.json')
    else:
    ...
    print(index_to_ans[0:2])
    for word in index_to_ans[0:2]:
    ...
    print(index_to_qw[0:2])
    for word in index_to_qw[0:2]:
